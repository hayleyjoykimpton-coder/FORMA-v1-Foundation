...
- Temporary **Life & Soul · Christmas Cracker** mode lives inside FORMA (Profile → Challenge mode). Same localStorage/Supabase — no second database. MOVE sub-nav: Training, Learn, My Progress, Recap, Fitness Testing, InBody + Measurements, Photos. CONNECT sub-nav: Community, Events, What's Included. Event dates/times stay on Facebook. External URLs live in `lib/crackerLinks.ts` (nutrition, Facebook, Jess Instagram, video slots, optional upgrades). Jess education ticks stay in `programme.moveChecklist`. My Progress only reads existing history/check-ins. Cracker Profile hides FORMA goal/training/body/preferences; Need Help is compact mailto (Hayley app support, Origin Wellness nutrition) / Jess / Facebook. CONNECT Community includes Thursday Question Bomb and Cracker of the Week.
