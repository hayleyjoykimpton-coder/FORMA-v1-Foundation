...
- Home UX: one dominant **Do this next** card (`lib/nextAction.ts`); secondary modules sit in collapsibles. Home module show/hide/reorder/**pin** + remembered open state is local (`lib/homePrefs.ts` · Customise Home). Training-day reminders are parked (no push / they did not work in-browser) — do not show Profile reminder settings or the Home training-day banner. Sunday weekly-review nudge on Home (`lib/weeklyReviewNudge.ts`). Progress Overview has CSV export (`lib/exportProgress.ts`).
- Progress uses a sticky sub-nav (Overview | Strength | Body | InBody | Photos). InBody scans live in `lib/inbody.ts` / `programme.inbody`. **Upload InBody screenshot** (Progress → InBody) posts to `POST /api/analyze-inbody` (same `OPENAI_API_KEY` as meals; default vision model **gpt-4o** with `detail: high`, tuned for InBody 270 sheets — screenshots/photos only). Prefer a flat gallery screenshot over a dim angled camera shot. Review filled fields → **Save scan**. Manual entry still works without AI. Training exercise/workout edit·swap·delete lives behind a ⋯ `ActionMenu`.
...
- Workout session: per-set Skip, exercise notes, rest −15/+15/Restart/Skip rest, finish confirms unfinished→skipped, then **Nice work** celebration (`sessionCelebration` in `FormaApp.tsx`). Body tab: Quick weight vs Weight+tape, tape empty CTAs, before/after photo pickers + swap (`ProgressPanel.tsx`). Coach copy stays quiet/Hayley-toned in `lib/coach.ts`, `lib/content.ts`, `lib/reminders.ts`.
- Flexible sessions: Training ↑↓ **swaps day labels** with the neighbour (`lib/workoutSchedule.ts`). Tap weekly schedule cards or **Choose session** to start any workout. **Move to today** swaps a missed session onto today’s weekday.
- Live workout sessions must stay bound to `session.workoutId` (not `pickTodaysWorkout`). Do **not** full-rehydrate on `TOKEN_REFRESHED`. Cloud pull **merges** history by session id (`lib/historyMerge.ts`) — never blind-replace with a shorter cloud array. Weekly schedule/Training cards show **✓ Done** for sessions logged this calendar week.
- Temporary **Life & Soul · Christmas Cracker** mode lives inside FORMA (Profile → Challenge mode). Same localStorage/Supabase — no second database. Uses `public/brand/christmas-cracker-*.png`, red/black accents, Home banner + nutrition guide. Paste real guide copy into `lib/crackerGuide.ts` when ready. MOVE sub-nav: Training, Recap, Fitness Testing, InBody + Measurements, Photos (week-by-week Front/Side/Back compare; same `forma-photos-v1` cloud blob). Cracker Profile shows account, name, club, and Beginner/Intermediate only — hide Goal, Training (days/equipment/location), Body stats, and Preferences until the post-season FORMA update. Jess videos: paste YouTube/Vimeo links into `CRACKER_INTRO_VIDEO` and `CRACKER_WEEKLY_TRAINING_VIDEOS` in `lib/jessTrainer.ts` — they play in-app. Education/intro ticks sync in `programme.moveChecklist`; workout completion is `user_state.history`. No coach dashboard yet — signed-in rows are visible in the Supabase table editor.
