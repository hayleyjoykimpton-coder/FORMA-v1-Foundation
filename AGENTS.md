...
- Temporary **Life & Soul · Cracker Challenge** mode lives inside FORMA (Profile → Challenge mode). Same localStorage/Supabase — no second database. Branding + Home banner + nutrition guide card; turn off to return to FORMA. Paste real guide copy into `lib/crackerGuide.ts` when ready.
