...
- Temporary **Life & Soul · Christmas Cracker** mode lives inside FORMA (Profile → Challenge mode). Same localStorage/Supabase — no second database. Uses `public/brand/christmas-cracker-*.png`, red/black accents, Home banner + nutrition guide. Paste real guide copy into `lib/crackerGuide.ts` when ready.
