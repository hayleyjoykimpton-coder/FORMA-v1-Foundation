...
- Visual trial (optional): white canvas + polished espresso/serenity tokens live in `app/globals.css` `:root`. To revert cream stone: restore `--bg: #e9e1d6` / `--card: #f3ebe2` and the warmer wash/shadow block from git history.
